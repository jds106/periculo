...
def pitch_loader() -> pd.DataFrame:
    ...
    df[['companies_house_status','companies_house_incorporated', 'companies_house_type']] = df.company_number.apply(lambda num: parser_companieshouse.parse(num)).apply(pd.Series)
